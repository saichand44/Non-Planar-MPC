...
def generate_surfaces():
    ...
    for percent in range(0, 125, 25):
        ...
        # a  = np.array([0, 0, 0,    -pi/6, -pi/3, -pi/3, -pi/6, 0, 0])
        # b  = a * 0
        # c  = np.array([0, 0, pi/12, pi/6, pi/12, -pi/12, -pi/6, -pi/12, 0]) * (percent/100)
        # config = TaitBryanSurfaceConfig(s = s, a = a ,b = b, c = c, y_min = -4, y_max = 4)
        # surf.initialize(config)
        # surf.save_surface(f'chicane_{percent}%')

        # surf = TaitBryanAngleSurface()
        # ds  = np.array([0,10, 10,     10,     10,      10,      10,      10,      10,       15,     20,       10])
        # s = np.cumsum(ds)
        # a = np.array([0,  0,      pi/2,   pi/2,   pi/2,   pi/4,   0,      0,      0,      -pi/2,  -pi/2,  -pi/2])
        # b = np.array([0,  0,      0,      0,      0.2,    0.4,    0.5,    0.4,    0.25,    0.1,     0,      0]) * (percent/100)
        # c = np.array([0,  -0.2,   -0.3,   -0.2,   0,      0.2,      0.3,    0.2,    0,      0.2,     0,      0]) * (percent/100)
        # config = TaitBryanSurfaceConfig(s = s, a = a ,b = b, c = c, y_min = -4, y_max = 4)
        # surf.initialize(config)
        # surf.save_surface(f'hill_climb_{percent}%')
        ...
        # a = np.array([0,0,0,pi/3, pi/3, pi/3])
        # b = 0 * a
        # c = np.array([0,0,pi/8,pi/8,0,0]) * (percent/100)
        # config = TaitBryanSurfaceConfig(s = s, a = a ,b = b, c = c, y_min = -4, y_max = 4)
        # surf.initialize(config)
        # surf.save_surface(f'off_cam_turn_{percent}%')
        ...
        # config.b = np.array([0,0,0,0,0,0])
        # config.c = np.array([0,0,0,0,0,0])
        # config.k = np.array([.001,.001,.099,0.099,.001,.001]) * (percent/100)
        # config.y_max = 2
        # config.y_min = -8
        # config.use_pchip = True
        # surf.initialize(config)
        # surf.save_surface(f'tube_turn_{percent}%')

        surf = TaitBryanAngleSurface()
        ds = np.array([0, 2, 1, 1, 2]) *10
        s = np.cumsum(ds)
        a = np.array([0, 0, 0, 0, 0])
        b = np.array([0, 0.5, 0, -0.5, 0]) * (percent/100)
        c = np.array([0, 0, 0, 0, 0])
        config = TaitBryanSurfaceConfig(s = s, a = a, b = b, c = c, y_min = -4, y_max = 4, use_pchip = False, closed = False)  
        surf.initialize(config)
        surf.save_surface(f'single_bump_{percent}%')
